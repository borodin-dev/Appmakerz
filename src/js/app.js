...
$(document).ready(function () {
    ...
    $('.accordionContent').hide();                  //CLOSES ALL S ON PAGE LOAD
    ...
    /* ====== Filtering Checkbox Options ====== */
    $('.content-sidebar').delegate('input[type=checkbox]', 'change', function() {
        let $lis = $('.content-logo__container > div'),
        ...
        if ($checked.length) {
            ...
            $('.content-logo__container > div').hide().filter(selector).show();
        } else {
            $lis.show();
        }
    });
    ...
    $('#sendForm').submit( function (e) {
        ...
        var mail = $('#zakelijkeEmail').val();
        var phone = $('#telefoonnummer').val();
        var companyName = $('#bedrijfsnaam').val();
        var text = $('#jouwBericht').val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(alertify.success('Success message')){
            ...
            error: function (data) {
                // alert("Error" + data);
                console.log(data);
                if(alertify.error('Error message')){
                    setTimeout(window.location.href = '', 3000);
                }
...
$(document).on('load', function () {
    /* ====== Select Options ====== */
    let brancheControlEl = document.getElementById('brancheControlEl');
    let selectEl = document.getElementById('selectEl');
    selectEl.onchange = detectSelectVal;
    function detectSelectVal(event) {
        let elVal = this.value;
        if (elVal) {
            brancheControlEl.classList.add('has-value');
        } else {
            brancheControlEl.classList.remove('has-value');
        }
    }
});
